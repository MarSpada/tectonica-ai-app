 ...
 * Super admin only. Uploads a logo image to Supabase Storage bucket "branding".
 * Accepts multipart form data with a "logo" file field.
 * Updates group_branding.logo_url with the public URL.
 *
...
export async function POST(request: Request) {
  try {
    ...
    const storagePath = `${profile.group_id}/logo.${ext}`;
    const buffer = Buffer.from(await file.arrayBuffer());

    // Upload to branding bucket (upsert to overwrite previous logo)
    ...
    // Upsert group_branding row with logo_url
    ...
    if (dbError) {
      console.error("Branding logo DB error:", dbError);
